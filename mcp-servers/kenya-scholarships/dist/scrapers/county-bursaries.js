        ...
        try {
            ...
            for (const path of possiblePaths) {
                try {
                    ...
                    $('.bursary, .scholarship, article, .card').each((index, element) => {
                        const $item = $(element);
                        const name = $item.find('h2, h3, .title, .name').text().trim() ||
            ...
            console.error(`Error scraping ${county} county bursaries:`, error.message);
            return [];
        }
    }
...
//# sourceMappingURL=county-bursaries.js.map